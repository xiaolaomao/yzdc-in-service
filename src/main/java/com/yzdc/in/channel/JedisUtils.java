...
public class JedisUtils {
    ...
    private static int MAX_IDLE = Integer.valueOf(PropertyTools.getPropertyBy("jedis.pool.maxIdle"));//控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值也是8。
    // 等待可用连接的最大时间，单位毫秒，默认值为-1，表示永不超时。如果超过等待时间，则直接抛出JedisConnectionException；
    private static int MAX_WAIT = Integer.valueOf(PropertyTools.getPropertyBy("jedis.pool.maxWait"));
    private static int TIMEOUT = Integer.valueOf(PropertyTools.getPropertyBy("jedis.pool.timeOut"));//超时时间
    // 在borrow一个jedis实例时，是否提前进行validate操作；如果为true，则得到的jedis实例均是可用的；
    private static boolean TEST_ON_BORROW = Boolean.valueOf(PropertyTools.getPropertyBy("jedis.pool.testOnBorrow"));
    // redis缓冲池
